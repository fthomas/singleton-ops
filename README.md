...
* Added `Id` to allow just referencing the same value without any modification.
...
```scala
import singleton.ops._
def demo[L <: XInt](implicit p : L*L + L) : p.Value {} = p.value
val b : 30 = demo[5]
```
* `Long` type operations:
```scala
import singleton.ops._
def demoLong[L1 <: XLong, L2 <: XLong](implicit p : Min[L1*L1, L2+L2]) : p.Value {} = p.value
...
```scala
import singleton.ops._
def demoDouble[L1 <: XDouble, L2 <: XDouble](implicit p : L1 / L2 + 1.0) : p.Value {} = p.value
...
```scala
import singleton.ops._
def demoSumLongInt[L1 <: XLong, L2 <: XInt](implicit p : L1 + L2) : p.Value {} = p.value
...
```scala
import singleton.ops._
def demoString[P1 <: XString](implicit op : Reverse[P1] + P1) : op.Value {} = op.value
...
```scala
import singleton.ops._
def demoBoolean[P1 <: XInt](implicit op : P1 < 0) : op.Value{} = op.value
...
```scala
import singleton.ops._
def demoRequire[P1 <: XInt](implicit op : Require[P1 < 0]) : op.Value{} = op.value
scala> demoRequire[-1]
demoRequire[-1]
...
def demoNatToSing[L <: Nat](implicit p : L+L) : p.Value {} = p.value
...
```scala
import singleton.ops._
def bigMul[L1 <: XLong, L2 <: XLong](implicit p : L1 * L2) : p.Value {} = p.value
...
[twoface-discourse]: https://contributors.scala-lang.org/t/twoface-values-closing-the-gap-between-run-compile-time-functionality/869